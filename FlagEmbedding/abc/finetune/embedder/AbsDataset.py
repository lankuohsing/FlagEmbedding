...
class AbsEmbedderEvalDataset(Dataset):
    """Abstract class for evaluation dataset.
    ...
    def __getitem__(self, item):
        data = self.dataset[item]
        eval_group_size = self.args.eval_group_size
        ...
        # pos_idx = random.choice(list(range(len(data['pos']))))
        pos_idx = 0#验证集应该保证每次结果是固定的，默认只有一个pos
        passages.append(self._shuffle_text(data['pos'][pos_idx]))

        neg_all_idx = list(range(len(data['neg'])))
        neg_idxs=neg_all_idx[0:min(len(neg_all_idx), eval_group_size-1)]
        for neg_idx in neg_idxs:
            passages.append(data['neg'][neg_idx])
...
class AbsEmbedderCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        # return_loss=False if self.current_mode=='train' else True
        return {
            "queries": q_collated,
            "passages": d_collated,
