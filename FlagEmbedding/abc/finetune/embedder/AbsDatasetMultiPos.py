...
class AbsEmbedderTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if pos_num==-1:
            pos_num=len(data['pos'])#如果pos_num==-1,则选取训练数据里面所有的pos参与训练，除非超过了train_group_size-1（此时会截断）
        pos_num=min(pos_num, train_group_size-1)#保证至少一个neg
        pos_all_idx=list(range(len(data['pos'])))
        pos_idxs=random.sample(pos_all_idx, pos_num)
        # pos_idx = random.choice(list(range(len(data['pos']))))
        for pos_idx in pos_idxs:
            passages.append(self._shuffle_text(data['pos'][pos_idx]))# 默认不shuffle

        neg_all_idx = list(range(len(data['neg'])))
        if len(data['neg']) < train_group_size - pos_num:#neg文档不足，需要循环采样
            num = math.ceil((train_group_size - pos_num) / len(data['neg']))
            neg_idxs = random.sample(neg_all_idx * num, train_group_size - pos_num)
        else:
            neg_idxs = random.sample(neg_all_idx, self.args.train_group_size - pos_num)
        ...
        return query, passages, teacher_scores,pos_num
...
class AbsEmbedderCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        pos_nums=[f[3] for f in features]
                ...
                tmp_passages = [
                    self.args.query_instruction_format.format(
                        batch_raw_data['prompt'][
                            i] if 'prompt' in batch_raw_data else self.args.query_instruction_for_retrieval,
                        p
